<template>
    <div class="p-1">
        <div class="form-group mt-1">
            <router-link :to="{name: 'products_brand_index'}" class="btn btn-dark btn-sm" title="Назад">{{
                    $t('default.back')
                }}
            </router-link>
        </div>
    ...
    <div v-else class=" mt-1">
        <h5 class="card-title mt-1">{{ $t('default.edit') }} {{ model.title }}</h5>
        <div class="p-1">
...
export default {
    ...
    methods: {
        getData: function () {
            ...
            app.model_id = id;
            axios.get('/api/v1/products_brands/' + id + '/get/')
                .then(function (resp) {
                    app.model = resp.data;
                    app.preloader = false;
                })
        ...
        saveForm(e) {
            ...
            axios.post('/api/v1/products_brands/' + app.model_id + '/update', newModel)
                .then(function (resp) {
        ...
        uploadThumb: function (event) {
            ...
            axios.post('/api/v1/products_brands/addimages', formData)
                .then(function (resp) {
