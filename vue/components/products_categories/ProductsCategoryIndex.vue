<template>
    ...
    <div v-else>
        <div class="mt-1">
            <h5 class="card-title">{{ $t('products.category', 2) }}</h5>
        </div>
        <div class="mt-1">
            <div class="form-group">
                <router-link :to="{name: 'products_category_create'}" class="btn btn-success">{{ $t('default.created') }}</router-link>
            </div>
        </div>
                ...
                <tr v-for="item, index in items.data">
                    ...
                    <td>
                        <router-link
                            ...
                            :to="{name: 'products_category_edit', params: {id: item.id}}"
...
export default {
    ...
    methods: {
        getResults: function () {
            ...
            axios.post('/api/v1/products_categories', this.itemssearch)
                .then(function (resp) {
                    app.items = resp.data;
                    app.search = false;
                })
        ...
        deleteEntry: function (item, index) {
            if (confirm(this.$t('alert.confirm_delete', {title:item.title}))) {
                var app = this;
                app.search = true;
                axios.delete('/api/v1/products_categories/' + item.id + '/destroy')
                    .then(function (resp) {
                        app.search = false;
